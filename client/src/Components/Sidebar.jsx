...
export const Sidebar=({ mode, setMode,  notification })=>{
    // const drawerWidth=240;
    const [anchorEl, setAnchorEl] = React.useState(null);
    const {setProfileUser, activeUser}=React.useContext(UserContext);
    const {setTriggerPost}=React.useContext(PostsContext);
...
setMessageOpen(true);
    ...
    return(
        ...
        <Box sx={{width:"100%"}}>
        <AccountsMenu setMode={setMode} mode={mode} notification={notification} anchorEl={anchorEl} setAnchorEl={setAnchorEl} setNotifications={setNotifications} onsearch={onsearch} notifications={notifications} setOnsearch={setOnsearch}/>
        <Toolbar />
        
              <Toolbar />
              {
              !messageOpen&&<Typography variant="h5" sx={{fontSize:"30px" , fontWeight:"bolder"}}>Connect Hub</Typography>}
